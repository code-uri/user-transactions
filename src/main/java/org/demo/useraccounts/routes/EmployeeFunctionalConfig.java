...
import org.demo.useraccounts.repository.EmployeeRepository;
...
public class EmployeeFunctionalConfig {
	...
	RouterFunction<ServerResponse> getEmployeeByIdRoute() {
		return route().GET("/accounts/{id}", findEmployeeByIdFunction(), findEmployeeByIdOpenAPI()).build();
	}
	...
	private HandlerFunction<ServerResponse> findEmployeeByIdFunction() {
		return req -> ok().body(
				userAccountRepository.findById(Long.parseLong(req.pathVariable("id"))), Employee.class);
	}
	...
	private Consumer<Builder> findEmployeeByIdOpenAPI() {
		return ops -> ops.tag("employee")
				.operationId("findById").summary("Find purchase order by ID").tags(new String[] { "MyEmployee" })
